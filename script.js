...
async function fetchData(){
    try {
        const binsResponse = await fetch("https://api.jsonbin.io/v3/c/65c513621f5677401f2cb475/bins/", {
          method: "GET",
          headers: {
            "X-Master-Key": "$2a$10$Qud2bXwUn9OkErPqUZ9sxOgkEOxDr5nGrk4ybLJyC1YwnavK4c5O2",
          },
        });
        ...
        for (const item of binsData) {
          if (item.record !== user_id) {
            try {
              const binResponse = await fetch(`https://api.jsonbin.io/v3/b/${item.record}`, {
                method: "GET",
                headers: {
                  "X-Master-Key": "$2a$10$Qud2bXwUn9OkErPqUZ9sxOgkEOxDr5nGrk4ybLJyC1YwnavK4c5O2",
                },
              ...
              card.className = "card border my-2 py-2 ps-2 flex items-center justify-between px-2 rounded-sm";
      
              card.innerHTML = `
                <div class="flex items-center">
                  <div class="overflow-hidden object-fit rounded-full" style="width: 50px;height: 50px;">
                    <img src="${data.record.image !== "" ? data.record.image : "https://www.pngall.com/wp-content/uploads/5/User-Profile-PNG-Image.png"}" alt="">
                  </div>
                  <div class="text-md ps-1">${data.record.name}</div>
                </div>
                <div class="flex">
                  <button class="p-1 rounded-sm me-2 border" id='checkBtn-${data.metadata.id}'><img src="./image/view.png" width="25px" height="25px" alt=""></button>
                  <button class="border text-white p-1 rounded flex items-center" id='giveBtn-${data.metadata.id}'><img src="./image/chocolate.png" width="20px" height="20px" alt=""></button>
                </div>`;
      
              itemList.appendChild(card);
      
              const checkButton = card.querySelector(`#checkBtn-${data.metadata.id}`);
